...
Thus, we define a c_+(x) as the amount to which the constraint is violated.
In this formulation, we us a projection definition of c_+(x).
c_+(x) = 2norm between the point x and the specific constraint

Then, ∇c_+(x) is defined piecewise:
...
Notice that c(x) ≤ 0 has c_+(x) = 0, so 2 c_+(x) ∇c_+(x) covers both cases.
